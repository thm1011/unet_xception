...
class Generator(object):
    def __init__(self,batch_size,train_lines,image_size,num_classes, img_dir, mask_dir):
    ...
    def aug_fn(self, image, label):
        transform = A.Compose([
            A.ShiftScaleRotate(border_mode=0, value=[255, 255, 255], mask_value=0),
    ...
    def generate(self, random_data = False):
        i = 0
        ...
        while True:
            ...
            # 从文件中读取图像
            jpg = Image.open(os.path.join(self.img_dir, name + ".png"))
            png = Image.open(os.path.join(self.mask_dir, name + ".png"))
#             jpg = jpg.resize((256,256),Image.ANTIALIAS)
#             png = png.resize((256,256),Image.ANTIALIAS)

            if random_data:
                # jpg, png = self.get_random_data(jpg,png,(int(self.image_size[1]),int(self.image_size[0])))
                jpg, png = self.aug_fn(np.array(jpg), np.array(png))
            else:
                jpg, png = letterbox_image(jpg, png, (int(self.image_size[1]),int(self.image_size[0])))
            
            inputs.append(np.array(jpg)/255)
            # inputs.append(jpg.astype('float32')/255)
